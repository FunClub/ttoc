local Bot = TTOCInitScript("冬蹄营地-生命结晶")
local Log = Bot.Log
local UI = Bot.UI
...
local mainScript = {
      ...
      {If = "SC:GetPlayer().Level >= 1 and SC:GetPlayer().Level < 80", End = '自定义打怪'},
            ...
            {Settings = 'BuyNPC', value = {
                  ...
                  --修装的
                  {Id =24028  ,Path = {2672.5325, -4306.2305, 290.9643}},
                   --食物商人    id是商人的npcid    后面的坐标是我站着对话商人的坐标
                   {Id =24033  ,Path = {2681.9358, -4390.9810, 284.3628}},
                  --弓箭商人
                  {Id =24028  ,Path = {2672.5325, -4306.2305, 290.9643}},
            }},
            ...
            --打怪循环设置
            {AttackMonster = {23874} , Count = 1 ,  --0000,0000是要打的怪物id
            MoveTo = {--0000.0000,0000.0000,0000.0000 是要巡逻的坐标
            { { 2428.3098, -4568.6802, 226.7115 }, },
            { { 2362.7341, -4573.9570, 221.2661}, },
            { { 2307.1272, -4617.3159, 217.7915 }, },
            { { 2376.0288, -4687.9160, 230.3312 }, },
...
Bot:SetStart(function()
     Config:Hide()
      ...
      --最先加载的线程初始化
      SetSettings({
            GatherBlackList = {{Name = "孢子", SkillReq = 999}},
            --自动采矿
            AutoGatherOre = true,
            --自动采药
            AutoGatherHerb = true,
		...
		StuckEvent = true,
           ...
           -- FoodPercent = {70 ,90 ,},
            ...
            --不贩卖列表 
            DoNotSellList = {'雪莲花','巫妖花','金苜蓿','蛇信草','冰棘草','塔兰德拉的玫瑰','卷丹','结晶','永恒','钴矿石','药剂','法力之油','巫师之油','美味风蛇','符文法力药水','传送','Hearthstone','炉石','Skinning Knife','剥皮小刀','Mining Pick','矿工锄','Primal','源生','Mote of','微粒','Air','空气','基尔加丹印记',"Mark of Kil'jaeden","Thieves'Tools",'潜行者工具','Flash Powder','闪光粉'},
            --强制贩卖列表 如果需要可以自行添加
            ForceSellList = {'蜜饯苔藓','冰霉果'},
            --强制销毁
